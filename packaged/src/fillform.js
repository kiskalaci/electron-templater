...
function updateTextValue(handles, fieldDictionary, value, isRich, inheritedProperties) {
    ...
    modifiedDict
        .writeKey('V')
        .writeLiteralStringValue(new muhammara.PDFTextString(value['v']).toBytesArray());

    if (isRich) {
        modifiedDict
            .writeKey('RV')
            .writeLiteralStringValue(new muhammara.PDFTextString(value['rv']).toBytesArray());
    }
...
function updateChoiceValue(handles, fieldDictionary, value, inheritedProperties) {
    ...
    if (typeof (value) === 'string') {
        ...
        modifiedDict
            .writeKey('V')
            .writeLiteralStringValue(new muhammara.PDFTextString(value).toBytesArray());
        textToWrite = value;
    }
    else {
        // multiple options
        ...
        value.forEach(function (singleValue) {
            handles.objectsContext.writeLiteralString(new muhammara.PDFTextString(singleValue).toBytesArray());
        });
...
function writeKidsAndEndObject(handles, parentDict, kidsArray) {
    ...
    fieldJSArray.forEach(function (field) {
        if (field.getType() === muhammara.ePDFObjectIndirectObjectReference) {
...
function fillForm(writer, data) {
    ...
    if (acroformInCatalog.getType() === muhammara.ePDFObjectIndirectObjectReference) {
